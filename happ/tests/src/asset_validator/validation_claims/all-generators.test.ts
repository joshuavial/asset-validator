...
import { createGenerator } from './common.js';

test('create a Generator and get all generators', async () => {
  await runScenario(async scenario => {
    ...
    const createdRecord: Record = await createGenerator(alice.cells[0]);
    assert.ok(createdRecord);
    
    await pause(1200);
    
    // Bob gets all generators again
